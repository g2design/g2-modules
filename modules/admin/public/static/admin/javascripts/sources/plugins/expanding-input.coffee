...
ExpandingInput = ($container, options={}) ->
  ...
  @$overlay.append($('<div class="expanding-input-placeholder"></div>').text(@options.placeholder)) if @options.placeholder
...
ExpandingInput.prototype.expand = () ->
  ...
  setTimeout(=>
    @$target.focus()
  , 1) if @$target
  @$target.attr('placeholder', @options.placeholder) if @$target and @options.placeholder
